...
setInterval(checkForPurchase, 15000); // Check every 15 seconds
    ...
    /**
     * NEW: POPUP MESSAGE SYSTEM WITH SPLIT TESTING SUPPORT
     */
    async function getPopupMessage() {
      // Get user ID for this chatbot
      ...
      if (!userId) {
        ...
        localStorage.setItem('userId_' + chatbotID, userId);
      }

      try {
        // Try to get popup message from database (with split testing support)
        const response = await fetch(`https://egendatabasebackend.onrender.com/api/popup-message/${chatbotID}?user_id=${encodeURIComponent(userId)}`);
        
        if (response.ok) {
          const data = await response.json();
          console.log('Popup message from API:', data);
          
          // Store split test info if user is part of one
          if (data.is_split_test) {
            localStorage.setItem(`splitTest_${chatbotID}`, JSON.stringify({
              split_test_id: data.split_test_id,
          ...
          console.warn('Failed to fetch popup message from API, using fallback');
        }
      } catch (error) {
        console.warn('Error fetching popup message from API:', error);
      }
      
      // Fallback to hardcoded value (backwards compatibility)
      // This will be used if the API is unavailable or returns an error
      return "Hej! Jeg hedder Elmer- jeg er GreenCarGears AI chatbot";
    }

    /**
     * TRACK POPUP EVENTS FOR SPLIT TESTING
     */
    async function trackPopupEvent(eventType) {
      var userId = localStorage.getItem('userId_' + chatbotID);
      if (!userId || !chatbotID) return;

      try {
        await fetch('https://egendatabasebackend.onrender.com/api/track-popup-click', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            user_id: userId,
        ...
        console.log(`Tracked popup event: ${eventType}`);
      } catch (error) {
        console.warn('Error tracking popup event:', error);
      }
    }
    ...
    function sendMessageToIframe() {
      ...
      pineconeApiKey: "",
      knowledgebaseIndexApiEndpoint: "greencargear-alt",
      flow2KnowledgebaseIndex: "",
      flow3KnowledgebaseIndex: "greencargear-pro",
      ...
      messageIcon: "https://raw.githubusercontent.com/DialogIntelligens/image-hosting/master/chatbot_message_icon/logo-1754480856236.png",
      themeColor: "#00a770",
      aiMessageColor: "#e6e6e6",
      aiMessageTextColor: "#000000",
      headerTitleG: "",
      headerSubtitleG: "Ved at bruge chatbotten accepterer du at der kan opstå fejl, og at samtalen gemmes og behandles. Har du behov for menneskelig hjælp, kan du altid kontakte vores kundeservice på mail: hey@greencargear.dk eller\n på telefonnummer: 70 60 40 61",
      subtitleLinkText: "",
      subtitleLinkUrl: "",
        
      titleG: "Support-botten Elmer",
      firstMessage: "Hej, mit navn er Elmer, jeg er GreenCarGears AI chatbot. Jeg kan hjælpe dig med produktspørgsmål eller generelle spørgsmål omkring vores webshop ⚡️",
...
function trackChatbotOpen() {
  ...
  .catch(function(error) {
    ...
    function toggleChatWindow() {
      ...
      // Close the popup when the chat is opened and track popup click
      if (!isCurrentlyOpen) {
        ...
        // Track popup click for split testing
    ...
    /**
     * 7. SHOW/HIDE POPUP - UPDATED TO USE DATABASE
     */
    async function showPopup() {
      ...
      // Get popup message from database (with split testing support)
      const popupText = await getPopupMessage();
      messageBox.innerHTML = `${popupText} <span id="funny-smiley">😊</span>`;    
      
      // Track popup shown event for split testing
