...
providerHint: "commercetools",
  auth: {
    type: "commercetoolsOAuth",
    credentials: {
      clientId: "PpRchVoadh-EOZVSM93udjN2", // Your CTP_CLIENT_ID
      clientSecret: "oF43LNYPaAgJkg-qqabTaDIQge1uIyfA", // Your CTP_CLIENT_SECRET
      tokenUrl: "https://auth.europe-west1.gcp.commercetools.com/oauth/token",
      scope: "view_orders:dilling--production view_published_products:dilling--production view_states:dilling--production"
    }
  },
  orderFetch: {
    urlTemplate: "https://api.europe-west1.gcp.commercetools.com/dilling--production/orders?where=orderNumber%3D%22{{ORDER_NUMBER}}%22%20and%20shippingAddress(email%3D%22{{ORDER_EMAIL}}%22)",
    method: "GET",
    headersTemplate: [
      { key: "Authorization", value: "Bearer {{AUTH_TOKEN}}" },
      { key: "Accept", value: "application/json" }
    ]
    // No bodyTemplate needed for GET
  },
  stateFetchConfig: { // For fetching detailed state names
    enabled: true,
    urlTemplate: "https://api.europe-west1.gcp.commercetools.com/dilling--production/states/{{STATE_ID}}",
    headersTemplate: [
      { key: "Authorization", value: "Bearer {{AUTH_TOKEN}}" },
      { key: "Accept", value: "application/json" }
    ],
    stateResponse: {
      namePath: { path: "name", locale: "da-DK" }, // Primary path for localized name
      keyPath: "key" // Fallback to state key
    }
  },
  orderDetailsMappings: {
    basePath: "results[0]", // The order object is usually the first in the 'results' array
    commercetoolsStateLookups: { // Paths to find state IDs within the order for lookup
        orderStateRefIdPath: "state.id",
        lineItemStateRefIdPath: "state[0].state.id"
    },
    fields: {
      orderNumber: "orderNumber",
      orderDate: "createdAt",
      // Status is derived using stateFetchConfig, fallback to rawOrderStatusFromOrder
      rawOrderStatusFromOrder: "orderState.key", // Path to the key of the order's state if no detailed lookup
      customerFirstName: "shippingAddress.firstName",
      customerLastName: "shippingAddress.lastName",
      customerEmail: "shippingAddress.email",
      customerPhone: { path: "shippingAddress.phone", fallbackPath: "shippingAddress.mobile"},
      shippingMethod: "shippingInfo.shippingMethodName",
      shippingStreet: "shippingAddress.streetName",
      shippingCity: "shippingAddress.city",
      shippingPostalCode: "shippingAddress.postalCode",
      shippingCountry: "shippingAddress.country",
      shippingDropPoint: "custom.fields.order-field-DeliveryDropPointName",
      shippingTrackingInfo: "custom.fields.order-field-ShipmentUrl",
      shippingDeliveredDate: "custom.fields.order-field-DeliveredSetDate",
      paymentTotalPrice: { path: "totalPrice.centAmount", isPrice: true }, // Will be divided by 100
      paymentCurrencyCode: "totalPrice.currencyCode",
      discountHasCode: { path: "discountCodes", transformation: { type: "booleanLength" } }, // True if discountCodes array has items
      discountDetailsArray: "discountCodes",
      discountDetailId: "discountCode.id", // Within discountCodes array items
      discountDetailState: "state",       // Within discountCodes array items
      invoiceUrl: "custom.fields.order-field-PdfUrl"
    },
    lineItems: {
      arrayPath: "lineItems",
      itemFields: {
        productName: { path: "name", locale: "da-DK" },
        quantity: "quantity",
        price: { path: "price.value.centAmount", isPrice: true },
        currencyCode: "price.value.currencyCode",
        // Item status derived using stateFetchConfig, fallback to rawItemStatus
        rawItemStatus: "state[0].state.key", // Path to the key of the item's state
        sku: "variant.sku",
        imagesArray: "variant.images",
        imageUrl: "url" // Within imagesArray items
      }
    },
