...
async function main() {
  const ctx: Context = {
    ...
    config: {
      ...
      checkoutConfirmationPagePatterns: "",
      checkoutPagePatterns: "",
...
function getBackendUrl({ ctx }: { ctx: Readonly<Context>}) {
...
(async function() {
  ...
  function parsePriceFromText(priceText: string) {
  ...
  function trackTotalPurchasePrice() {
    ...
    if (amount) {
      // Track amount in local storage incase confirmation page is used as tracking indicator (or any other page)
      localStorage.setItem(purchaseTotalPriceKey(chatbotUserId), String(amount));
    }
  }
