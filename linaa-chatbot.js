    ...
    function purchaseKey(userId) {
      ...
      console.log('initChatbot called, checking DOM readiness...');
      
      // Check if DOM is fully ready and elements exist
      ...
      function toggleChatWindow() {
        ...
        console.log("Elements found - iframe:", !!iframe, "button:", !!button, "popup:", !!popup);
              ...
              // Better state detection - check multiple factors
      var isCurrentlyOpen = iframe.style.display !== 'none' && 
                           iframe.style.zIndex === '40000' && 
                           iframe.style.pointerEvents === 'auto' &&
                           button.style.display === 'none';
      
      console.log("isCurrentlyOpen:", isCurrentlyOpen, "iframe display:", iframe.style.display, "z-index:", iframe.style.zIndex, "pointer-events:", iframe.style.pointerEvents, "button display:", button.style.display);
      ...
      window.addEventListener('resize', function() {
        ...
        // If chatbot elements are missing after resize, try to reinitialize
        if (!document.getElementById('chat-container')) {
          console.log("Chatbot missing after resize, reinitializing...");
          setTimeout(function() {
            if (!document.getElementById('chat-container')) {
      ...
      // Enhanced element initialization with verification
      function setupChatElements() {
        var iframe = document.getElementById('chat-iframe');
        var button = document.getElementById('chat-button');
        
        if (!iframe || !button) {
          console.log('Chat elements not ready, retrying...', {iframe: !!iframe, button: !!button});
          setTimeout(setupChatElements, 100);
          return;
        }
        
        console.log('Chat elements ready, setting up...');
        
        // Force to correct initial state first
        var savedState = localStorage.getItem('chatWindowState');
        console.log('Saved chat state:', savedState);
        
        if (savedState === 'open') {
          console.log('Restoring chat to open state');
          iframe.style.display = 'block';
          iframe.style.zIndex = '40000';
          iframe.style.pointerEvents = 'auto';
          button.style.display = 'none';
          ...
          console.log('Setting chat to closed state');
          iframe.style.display = 'none';
          iframe.style.zIndex = '-1';
          iframe.style.pointerEvents = 'none';
          button.style.display = 'block';
      ...
      // Chat button setup is now handled in setupChatElements() function above
    ...
    // Add function to force chatbot to closed state
    window.forceChatbotClosed = function() {
      console.log("Forcing chatbot to closed state...");
      var iframe = document.getElementById('chat-iframe');
      var button = document.getElementById('chat-button');
      ...
      if (iframe && button) {
        ...
        button.style.display = 'block';
        localStorage.setItem('chatWindowState', 'closed');
        console.log("Chatbot forced to closed state");
        debugIframeState();
      } else {
        console.error("Could not find iframe or button elements");
      }
    };
    ...
    // Improved initialization with multiple retry attempts
    function initializeChatbotWithRetry() {
      var maxRetries = 5;
      var currentRetry = 0;
      
      function attemptInit() {
        console.log(`Chatbot initialization attempt ${currentRetry + 1}/${maxRetries}`);
        
        if (document.getElementById('chat-container')) {
          console.log("Chatbot already loaded successfully");
          return;
        }
          ...
          setTimeout(function() {
            if (document.getElementById('chat-container')) {
              ...
              if (currentRetry < maxRetries) {
                console.log(`Chatbot load failed, retrying in ${currentRetry * 1000}ms...`);
                setTimeout(attemptInit, currentRetry * 1000);
              } else {
                console.error("Chatbot failed to load after all retries");
              }
            }
          }, 1000);
    ...
    window.reinitializeChatbot = function() {
      console.log("Manual chatbot reinitialization requested");
      // Remove existing chatbot if present
      var existingContainer = document.getElementById('chat-container');
